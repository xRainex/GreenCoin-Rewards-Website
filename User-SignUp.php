                    ...
                    <div class="error-message-container">
                        ...
                        <form method="post" id="direct-verification">
                            ...
                            <div style="display:flex; justify-content:center; align-items:center; gap:20px;">
                                <button style="width:40%;" class="close" type="button">Back</button>
        ...
        if ($emailPVResult['countEmail'] > 0){
            if (trim($address) == "" && $phone_no == ""){
                $updateUserQuery = mysqli_query($conn, "UPDATE user SET username='$username', password = '$password', dob = '$dob', address = NULL, 
                    phone_number = NULL, points = '0', profile_image =  'User-Profile-Avatar1.png', created_at = NOW() WHERE email = '$email'");
            }else if ($phone_no == ""){
                $updateUserQuery = mysqli_query($conn, "UPDATE user SET username='$username', password = '$password', dob = '$dob', address = '$address', 
                    phone_number = NULL, points = '0', profile_image =  'User-Profile-Avatar1.png', created_at = NOW() WHERE email = '$email'");
            }else if (trim($address) == "" ){
                $updateUserQuery = mysqli_query($conn, "UPDATE user SET username='$username', password = '$password', dob = '$dob', address = NULL, 
                    phone_number = '$phone_no', points = '0', profile_image =  'User-Profile-Avatar1.png', created_at = NOW() WHERE email = '$email'");
            }else{
                $updateUserQuery = mysqli_query($conn, "UPDATE user SET username='$username', password = '$password', dob = '$dob', address = '$address', 
                    phone_number = '$phone_no', points = '0', profile_image =  'User-Profile-Avatar1.png', created_at = NOW() WHERE email = '$email'");
            }
            ...
            if (trim($address) == "" && $phone_no == ""){
                $addUserQuery = "INSERT INTO user(username, email, password, dob, address, phone_number, points, profile_image, created_at, status) VALUES
                    ('$username','$email','$password','$dob',NULL, NULL,'0', 'User-Profile-Avatar1.png', NOW(), 'Pending Verification')";
            }else if ($phone_no == ""){
                $addUserQuery = "INSERT INTO user(username, email, password, dob, address, phone_number, points, profile_image, created_at, status) VALUES
                    ('$username','$email','$password','$dob','$address', NULL,'0', 'User-Profile-Avatar1.png', NOW(), 'Pending Verification')";
            }else if (trim($address) == "" ){
                $addUserQuery = "INSERT INTO user(username, email, password, dob, address, phone_number, points, profile_image, created_at, status) VALUES
                    ('$username','$email','$password','$dob', NULL,'$phone_no','0', 'User-Profile-Avatar1.png', NOW(), 'Pending Verification')";
            }else{
                $addUserQuery = "INSERT INTO user(username, email, password, dob, address, phone_number, points, profile_image, created_at, status) VALUES
                    ('$username','$email','$password','$dob','$address','$phone_no','0', 'User-Profile-Avatar1.png', NOW(), 'Pending Verification')";
            }
...
<head>
    ...
    <style>
        ...
        *{
        ...
        .popup-error button {
            /* background: linear-gradient(145deg, #7bc74d, #5a8e36); */
    ...
    <script>
        ...
        phoneInput.addEventListener("keydown", function (event) {
            if (phoneInput.selectionStart <= 4 && (event.key === "Backspace" || event.key === "Delete")) {
                event.preventDefault(); // Prevent deletion of "+60 "
            }
        });
