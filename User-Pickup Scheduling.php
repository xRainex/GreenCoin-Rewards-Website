<?php
    ...
    if (isset($_GET['date'])) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            // $text = $formattedTime . " - " . date("h:i a", $timestamp + 60 * 60);
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (mysqli_query($conn, $addNewPickupRequestQuery)) {
            ...
            echo '<script>window.location.href = "User-Pickup Scheduling.php"; </script>';
        } else {
...
<body>
                        ...
                        <div class="pickup-item-quantity-div">
                            <div class="item-div">
                                ...
                                <select name="item_1">
                                    ...
                                    <?php 
                                        $getAllItemQuery = mysqli_query($conn, "SELECT item_name FROM item ORDER BY item_name");
    ...
    <div class="stickman-container">
        <img style="margin-left: 500px; margin-bottom: -15px; margin-top:50px; z-index:-1;" src="User-PickupScheduling-Content-Stickman1.svg" width="600">
    </div>
    ...
    <script>
        ...
        // Example event data (you can load from a database)
        // let events = {
        //     "2025-04-10": ["Meeting at 10 AM"],
        //     "2025-04-15": ["Project deadline"],
        //     "2025-04-22": ["Holiday trip"]
        // };
        ...
        async function fetchEvents() {
            try {
                ...
                events = await response.json(); // Convert response to JSON
            } catch (error) {
            ...
            for (let i = firstDay - 1; i >= 0; i--) {
                ...
                if (events[dateKey]) {
                    events[dateKey].forEach(event => {
                        ...
                        eventItem.textContent = event.content;
                        eventItem.classList.add('event-item');

                        let eventDate = new Date(dateKey);
                        const eventIsDisabled = event.status.includes("disabled") || eventDate < today;
            ...
            for (let i = 1; i <= nextDays; i++) {
                ...
                if (events[dateKey]) {
                    ...
                    events[dateKey].forEach(event => {
                        ...
                        let eventDate = new Date(dateKey);
                        const eventIsDisabled = event.status.includes("disabled") || eventDate < today;
                        console.log(`Event on ${dateKey}:`, event, "Is Disabled:", eventIsDisabled);
                        if (eventIsDisabled) {
                            eventItem.classList.add('disabled-event');
                        }
